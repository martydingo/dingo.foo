...
export function TagFilter({ tags, selectedTag, tagCounts }: TagFilterProps) {
    ...
    const DesktopTagFilter = () => (
        <div className="hidden md:flex flex-wrap gap-2">
            {tags.map((tag) => {
                const presentableTag = tag.replaceAll("-", " ").split(" ").map((tagWord) => `${tagWord[0].toUpperCase()}${tagWord.slice(1)}`).join(" ")
                return (
                    <button
                        key={tag}
                        onClick={() => handleTagClick(tag)}
                        className={`h-8 flex items-center px-1 pl-3 rounded-lg cursor-pointer border text-xs transition-colors ${selectedTag === tag
                            ? "border-primary bg-primary text-primary-foreground"
                            : "border-border hover:bg-muted"
                            }`}
                    >
                        <span>{presentableTag}</span>
                        {tagCounts?.[tag] && (
                            <span
